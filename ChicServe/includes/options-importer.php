...
if ( !class_exists( 'ACE_WP_Options_Importer' ) ) :

class ACE_WP_Options_Importer {
	...
	public function __clone() { wp_die( "Please don't __clone ACE_WP_Options_Importer" ); }

	public function __wakeup() { wp_die( "Please don't __wakeup ACE_WP_Options_Importer" ); }

	public static function instance() {
		if ( ! isset( self::$instance ) ) {
			self::$instance = new ACE_WP_Options_Importer;
			self::$instance->setup();
		}
	...
	public function register_importer() {
		if ( function_exists( 'register_importer' ) ) {
			register_importer( 'wp-options-import', __( 'Options', 'ace' ), __( 'Import wp_options from a JSON file', 'ace' ), array( $this, 'dispatch' ) );
		}
	}
	...
	public function export_filters() {
		?>
		<p><label><input type="radio" name="content" value="options" /> <?php _e( 'Options', 'ace' ); ?></label></p>
		<?php
	}
		...
		switch ( intval( $_GET['step'] ) ) {
			...
			case 1:
				...
				if ( $this->handle_upload() ) {
					...
					echo '<p><a href="' . esc_url( admin_url( 'admin.php?import=wp-options-import' ) ) . '">' . __( 'Return to File Upload', 'ace' ) . '</a></p>';
				}
				break;
			case 2:
	...
	private function header() {
		echo '<div class="wrap">';
		echo '<h2>' . __( 'Import WordPress Options', 'ace' ) . '</h2>';
	}
	...
	private function greet() {
		...
		echo '<p>'.__( 'Howdy! Upload your WordPress options JSON file and we&#8217;ll import the desired data. You&#8217;ll have a chance to review the data prior to import.', 'ace' ).'</p>';
		echo '<p>'.__( 'Choose a JSON (.json) file to upload, then click Upload file and import.', 'ace' ).'</p>';
	...
	private function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			return $this->error_message(
				__( 'Sorry, there has been an error.', 'ace' ),
				esc_html( $file['error'] )
			);
		}
		...
		if ( ! isset( $file['file'], $file['id'] ) ) {
			return $this->error_message(
				__( 'Sorry, there has been an error.', 'ace' ),
				__( 'The file did not upload properly. Please try again.', 'ace' )
			);
		}
		...
		if ( ! file_exists( $file['file'] ) ) {
			...
			return $this->error_message(
				__( 'Sorry, there has been an error.', 'ace' ),
				sprintf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'ace' ), esc_html( $file['file'] ) )
			);
		}
		...
		if ( ! is_file( $file['file'] ) ) {
			...
			return $this->error_message(
				__( 'Sorry, there has been an error.', 'ace' ),
				__( 'The path is not a file, please try again.', 'ace' )
			);
		}
	...
	private function pre_import() {
		...
		<form action="<?php echo admin_url( 'admin.php?import=wp-options-import&amp;step=2' ); ?>" method="post">
			...
			<h3><?php _e( 'What would you like to import?', 'ace' ) ?></h3>
			<p>
				<label><input type="radio" class="which-options" name="settings[which_options]" value="default" checked="checked" /> <?php _e( 'Default Options','ace' ); ?></label>
				<br /><label><input type="radio" class="which-options" name="settings[which_options]" value="all" /> <?php _e( 'All Options','ace' ); ?></label>
				<br /><label><input type="radio" class="which-options" name="settings[which_options]" value="specific" /> <?php _e( 'Specific Options','ace' ); ?></label>
			</p>

			<div id="option_importer_details">
				<h3><?php _e( 'Select the options to import', 'ace' ); ?></h3>
				<p>
					<a href="#" class="options-bulk-select" data-select="all"><?php _e( 'Select All', 'ace' ); ?></a>
					| <a href="#" class="options-bulk-select" data-select="none"><?php _e( 'Select None', 'ace' ); ?></a>
					| <a href="#" class="options-bulk-select" data-select="defaults"><?php _e( 'Select Defaults', 'ace' ); ?></a>
				</p>
				<table id="importing_options">
					<thead>
						<tr>
							<th>&nbsp;</th>
							<th><?php _e( 'Option Name', 'ace' ); ?></th>
							<th><?php _e( 'New Value', 'ace' ) ?></th>
						</tr>
					</thead>
					<tbody>
			...
			<h3><?php _e( 'Additional Settings', 'ace' ); ?></h3>
			<p>
				<input type="checkbox" value="1" name="settings[override]" id="override_current" checked="checked" />
				<label for="override_current"><?php _e( 'Override existing options', 'ace' ); ?></label>
			</p>
			<p class="description"><?php _e( 'If you uncheck this box, options will be skipped if they currently exist.', 'ace' ); ?></p>

			<div class="error inline" id="import_all_warning">
				<p class="description"><?php _e( 'Caution! Importing all options with the override option set could break this site. For instance, it may change the site URL, the active theme, and active plugins. Only proceed if you know exactly what you&#8217;re doing.', 'ace' ); ?></p>
			</div>

			<?php submit_button( __( 'Import Selected Options', 'ace' ) ); ?>
		</form>
		<?php
	}
	...
	private function import() {
		if ( $this->run_data_check() ) {
			if ( empty( $_POST['settings']['which_options'] ) ) {
				$this->error_message( __( 'The posted data does not appear intact. Please try again.', 'ace' ) );
				$this->pre_import();
				return;
			}
			...
			if ( 'all' == $_POST['settings']['which_options'] ) {
				...
				if ( empty( $_POST['options'] ) ) {
					$this->error_message( __( 'There do not appear to be any options to import. Did you select any?', 'ace' ) );
					$this->pre_import();
					return;
				}
			...
			foreach ( (array) $options_to_import as $option_name ) {
				if ( isset( $this->import_data['options'][ $option_name ] ) ) {
					if ( in_array( $option_name, $blacklist ) ) {
						echo "\n<p>" . sprintf( __( 'Skipped option `%s` because a plugin or theme does not allow it to be imported.', 'ace' ), esc_html( $option_name ) ) . '</p>';
						continue;
					}
					...
					if ( defined( 'WP_OPTION_IMPORT_BLACKLIST_REGEX' ) && preg_match( WP_OPTION_IMPORT_BLACKLIST_REGEX, $option_name ) ) {
						echo "\n<p>" . sprintf( __( 'Skipped option `%s` because this WordPress installation does not allow it.', 'ace' ), esc_html( $option_name ) ) . '</p>';
						continue;
					}
					...
					if ( ! $override ) {
						...
						if ( $old_value !== $hash ) {
							echo "\n<p>" . sprintf( __( 'Skipped option `%s` because it currently exists.', 'ace' ), esc_html( $option_name ) ) . '</p>';
							continue;
						}
					}
					...
					echo "\n<p>" . sprintf( __( 'Failed to import option `%s`; it does not appear to be in the import file.', 'ace' ), esc_html( $option_name ) ) . '</p>';
				}
			}

			$this->clean_up();
			echo '<p>' . __( 'All done. That was easy.', 'ace' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'ace' ) . '</a>' . '</p>';
		}
	}
	...
	private function run_data_check() {
		if ( empty( $this->import_data['version'] ) ) {
			$this->clean_up();
			return $this->error_message( __( 'Sorry, there has been an error. This file may not contain data or is corrupt.', 'ace' ) );
		}

		if ( $this->import_data['version'] < $this->min_version ) {
			$this->clean_up();
			return $this->error_message( sprintf( __( 'This JSON file (version %s) is not supported by this version of the importer. Please update the plugin on the source, or download an older version of the plugin to this installation.', 'ace' ), intval( $this->import_data['version'] ) ) );
		}

		if ( $this->import_data['version'] > self::VERSION ) {
			$this->clean_up();
			return $this->error_message( sprintf( __( 'This JSON file (version %s) is from a newer version of this plugin and may not be compatible. Please update this plugin.', 'ace' ), intval( $this->import_data['version'] ) ) );
		}

		if ( empty( $this->import_data['options'] ) ) {
			$this->clean_up();
			return $this->error_message( __( 'Sorry, there has been an error. This file appears valid, but does not seem to have any options.', 'ace' ) );
		}
...
ACE_WP_Options_Importer::instance();
